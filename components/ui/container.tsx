...
const Container = React.forwardRef<HTMLDivElement, ContainerProps>(
    ({ className, spacing, spacingTop, spacingBottom, align, justify, as = "section", asChild = false, ...props }, ref) => {
        const Comp = asChild ? Slot : as

        return (
            <Comp
                className={cn(containerVariants({ spacing, spacingTop: !spacing && spacingTop, spacingBottom: !spacing && spacingBottom, align, justify, className }))}
                ref={ref}
                {...props}
            >
                {props.children}
...
const containerContentVariants = cva(
    'w-full min-w-0',
    {
        variants: {
            area: {
...
const ContainerContent = React.forwardRef<HTMLDivElement, ContainerContentProps>(
    ({ className, area, areaStart, areaEnd, maxWidth, as = "div", asChild = false, ...props }, ref) => {
        const Comp = asChild ? Slot : as

        return (
            <Comp
                className={cn(containerContentVariants({ area, areaStart: !area && areaStart, areaEnd: !area && areaEnd, maxWidth, className }))}
                ref={ref}
                {...props}
            >
                {props.children}
...
export { Container, ContainerContent, containerContentVariants, containerVariants };
